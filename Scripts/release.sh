...
# Без --generate-notes: автосписок GitHub перечисляет все PR после прошлого тега
# подряд, а релиз описывает бандл, а не историю репозитория. Спеки, тесты, CI и
# инструменты разработки в артефакт не попадают, и в списке того, что получил
# пользователь, им не место. Раздел «Что изменилось» дописывается в заметки
# отдельно — из коммитов, тронувших исходники артефакта (.claude/skills/release).
...
if [ ${#DRAFT[@]} -gt 0 ]; then
    echo
    echo "Создан черновик — пока с одной инструкцией по установке. Допиши раздел"
    echo "«Что изменилось», вычитай заметки и нажми Publish release: тег $TAG"
